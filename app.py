...
def crater_diameter_m(d_m: float, v_mps: float, rho_i: float = RHO_DEFAULT, rho_t: float = 2000.0) -> float:
    # Gravity-dominated scaling (simplified, vertical impact)
    k = 1.3
    return k * (g_earth ** -0.22) * (v_mps ** 0.44) * ((rho_i / rho_t) ** (1.0/3.0)) * (d_m ** 0.78)
...
def delta_v_kinetic(m_ast: float, m_imp: float, v_imp_mps: float, beta: float = 1.5) -> float:
    if m_ast <= 0: return 0.0
    return (beta * m_imp * v_imp_mps) / m_ast

def delta_v_gravity_tractor(m_ast: float, m_trac: float, stand_off_m: float, tug_time_s: float) -> float:
    if stand_off_m <= 0: return 0.0
...
def estimate():
    ...
    return jsonify({
        "derived": {
            ...
            "delta_v_kinetic_mps": delta_v_kinetic(m_ast, imp_mass, imp_vel_kms*1000.0, beta),
            "delta_v_tractor_mps": delta_v_gravity_tractor(m_ast, tractor_mass, stand_off_m, tug_days*86400.0),
        },
...
def demo_impactor():
    return {
        ...
        "elements": {  # heliocentric (toy) for visualization
            "a_AU": 1.15, "e": 0.21, "i_deg": 6.5, "Omega_deg": 44.0, "omega_deg": 210.0,
            "M_deg": 123.0, "epoch_jd": 2460600.5
        },
