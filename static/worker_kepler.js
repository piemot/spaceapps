...
self.onmessage = (evt) => {
  const { elements, spanDays=365, stepDays=3, startOffsetDays=0, applyDv=null } = evt.data || {};
  if (!elements) { self.postMessage({ error: "Missing elements" }); return; }
  ...
  try {
    ...
    if (applyDv && applyDv.dv_mps > 0){
      ...
      shifted = positions.map(([x,y,z], i) => i===0 ? [x,y,z] : [x + lateral*1e-6, y, z]); // toy lateral offset
    }
