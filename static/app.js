...
const state = { neo: null, gameEnabled: false, mission: "std", interceptDays: 180, browseCache: [] };
  ...
  try {
    ...
    const items = (data.near_earth_objects || []).map(ne => ({
      ...
      diamM: ne.estimated_diameter?.meters ?
        [ne.estimated_diameter.meters.estimated_diameter_min, ne.estimated_diameter.meters.estimated_diameter_max] : null,
...
$("neoSearch").addEventListener("input", () => {
  ...
  const filtered = state.browseCache.filter(it => (it.name||"").toLowerCase().includes(q));
  renderBrowseList(filtered);
});

function renderBrowseList(items){
  neoListEl.innerHTML = "";
  if (!items.length){ neoListEl.textContent = "No matches."; return; }
  items.forEach(it => {
    const row = document.createElement("div"); row.className = "list-item";
    const left = document.createElement("div");
    left.innerHTML = `<b>${it.name}</b> <span class="badge">H ${num(it.h,1)}</span>` +
      (it.diamM ? ` <span class="badge">${num(it.diamM[0],0)}–${num(it.diamM[1],0)} m</span>` : "");
...
async function loadNEObyId(id){
  try {
    ...
    const dEst = raw.estimated_diameter?.meters || raw.estimated_diameter?.kilometers;
    let minM = 120, maxM = 220;
    if (dEst) {
...
(function initThree(){
  ...
  const camera = new THREE.PerspectiveCamera(55, wrap.clientWidth/wrap.clientHeight, 1e7, 5e13);
  camera.position.set(0, 0, 5e11);

  // simple orbit camera controls
  const target = new THREE.Vector3(0,0,0);
  let ctl = { rotating:false, lastX:0, lastY:0, yaw:0, pitch: Math.PI/2, dist: camera.position.length() };
  const sph = new THREE.Spherical().setFromVector3(camera.position.clone());
  ctl.yaw = sph.theta; ctl.pitch = sph.phi; ctl.dist = sph.radius;
  function applyCam(){
    const maxP = Math.PI-0.001, minP = 0.001;
  ...
  dom.addEventListener('mousedown', e => { if (e.button===0) ctl.rotating=true; ctl.lastX=e.clientX; ctl.lastY=e.clientY; e.preventDefault(); });
  dom.addEventListener('mouseup', ()=>{ ctl.rotating=false; });
  dom.addEventListener('mousemove', e=>{
    if (!ctl.rotating) return;
    const dx=e.clientX-ctl.lastX, dy=e.clientY-ctl.lastY; ctl.lastX=e.clientX; ctl.lastY=e.clientY;
    ctl.yaw -= dx*0.005; ctl.pitch -= dy*0.005; applyCam();
  });
  dom.addEventListener('wheel', e=>{ ctl.dist *= (1 + Math.sign(e.deltaY)*0.1); ctl.dist = Math.max(5e9, Math.min(1e13, ctl.dist)); applyCam(); }, { passive:true });

  // lights + context
  const ambient = new THREE.AmbientLight(0xffffff, 0.9); scene.add(ambient);
  const sun = new THREE.Mesh(new THREE.SphereGeometry(1.2e10, 32, 32), new THREE.MeshBasicMaterial({ color: 0xffd27f }));
  scene.add(sun);
...
function propagateOrbit(dv=null){
  ...
  if (!worker){ worker = new Worker("/worker_kepler.js"); }
  ...
  worker.postMessage({ elements: el, spanDays: 540, stepDays: 3, startOffsetDays: 0, applyDv: dv });
}
...
function drawOrbit(positions, earth, shifted){
  ...
  function makeLine(points, color){
    ...
    for (let i=0;i<points.length;i++){ arr[3*i]=points[i][0]; arr[3*i+1]=points[i][1]; arr[3*i+2]=points[i][2]; }
...
// ---------- Leaflet map ----------
(function initMap(){
  if (typeof L === "undefined") { console.warn("Leaflet not loaded; skipping map."); return; }
  ...
  map.on("click", (e)=>{
    ...
    impactMarker = L.marker([lat, lng]).addTo(map).bindPopup(`Impact point<br/>Lat ${lat.toFixed(2)}, Lng ${lng.toFixed(2)}`).openPopup();
  });
})();

// ---------- Metrics & Gamification ----------
$("gameEnabled").addEventListener("change", (e) => { state.gameEnabled = e.target.checked; renderGame(); });
$("missionSelect").addEventListener("change", (e) => { state.mission = e.target.value; renderGame(); });
$("interceptDays").addEventListener("input", (e) => { state.interceptDays = Math.max(10, Number(e.target.value)||180); renderGame(); });
...
function num(n, d=2){ return Intl.NumberFormat(undefined, { maximumFractionDigits: d }).format(n); }
...
async function estimate(){
  ...
  try {
    ...
    if (state.gameEnabled){
      ...
      const t = days * 86400;
      const reqDv = (R_earth_m / t) * (state.mission === "easy" ? 0.8 : state.mission === "hard" ? 1.2 : 1.0);
      ...
      renderGame({ requiredDv: reqDv, availableDv: haveDv, days, success, score });
...
function renderGame(extra){
  ...
  const base = [
    line("Mission", state.mission === "easy" ? "Easy — City Saver" : state.mission === "hard" ? "Hard — Planet Saver" : "Standard — Country Saver"),
    line("NEO", neoName),
